...
function showToast(msg, type, ms) {
  ...
  el.textContent = msg;
  host.appendChild(el);
  setTimeout(function() { el.remove(); }, ms + 400);
}
...
const DEFAULT_PROMPTS = [
  'A cat running a coffee shop','The last slice of pizza on earth',
  'A robot learning to dance','A haunted vending machine',
  'Two ghosts arguing over a remote','A dragon afraid of fire',
  'An octopus playing every instrument','A banana riding a skateboard',
  'A wizard who lost his hat','A spy disguised as a houseplant',
  'A penguin at the beach','A dog trying to use a computer',
  'A snail winning a race','A cloud having a bad day','Pizza delivery to the moon'
];
...
function _renderStroke(ctx, s, size) {
  if (s.tool === 'fill') { _floodFill(ctx, s.x, s.y, s.color, size); return; }
...
function drawingToDataURL(drawingData) {
  ...
  (drawingData.strokes || []).forEach(function(s) { _renderStroke(ctx, s, size); });
